...
class Neo4jRecommendationEngine:
    ...
    def _get_smart_recommendations(self, user_id, limit=10):
        ...
        if user_profile['total_ratings'] == 0:
            # New user - show diverse popular movies
            return self._get_diverse_popular_movies(limit)
        elif user_profile['action_preference'] > 0.3:
            # User likes action - get specialized action recommendations
            return self._get_action_recommendations(user_id, limit)
        elif user_profile['total_ratings'] < 5:
            # Light user - content-based recommendations
            return self._get_content_based_recommendations(user_id, limit)
        else:
            # Experienced user - hybrid approach
            return self._get_hybrid_recommendations(user_id, limit)
    ...
    def _get_hybrid_recommendations(self, user_id, limit=10):
        ...
        for i, movie in enumerate(collaborative):
            score = (len(collaborative) - i) / len(collaborative) * 0.4
            movie_scores[movie['movie_id']] += score
        
        # Weight content-based (40%)
        for i, movie in enumerate(content_based):
            score = (len(content_based) - i) / len(content_based) * 0.4
            movie_scores[movie['movie_id']] += score
        
        # Weight trending (20%)
        for i, movie in enumerate(trending):
            score = (len(trending) - i) / len(trending) * 0.2
            movie_scores[movie['movie_id']] += score
        
        # Combine all movies and sort by score
        all_movies = {m['movie_id']: m for m in collaborative + content_based + trending}
        sorted_movies = sorted(movie_scores.items(), key=lambda x: x[1], reverse=True)
        
        result = []
        for movie_id, score in sorted_movies[:limit]:
            if movie_id in all_movies:
                movie = all_movies[movie_id].copy()
    ...
    def _analyze_user_profile(self, user_id):
        ...
        WITH u, collect(r) as ratings, collect(m) as movies
        
        WITH u, ratings, movies,
             size(ratings) as total_ratings,
             CASE WHEN size(ratings) > 0 THEN avg([rating IN ratings | rating.rating]) ELSE 0.0 END as avg_rating,
             size([movie IN movies WHERE 'Action' IN movie.genres]) as action_movies
        
        RETURN total_ratings,
               avg_rating,
               action_movies,
               CASE WHEN total_ratings > 0 THEN toFloat(action_movies) / total_ratings ELSE 0.0 END as action_preference
        """
        ...
        if result:
            record = result[0]
            return {
                'total_ratings': record.get('total_ratings', 0),
                'avg_rating': record.get('avg_rating', 0.0),
                'action_movies': record.get('action_movies', 0),
                'action_preference': record.get('action_preference', 0.0)
            }
        return {'total_ratings': 0, 'avg_rating': 0.0, 'action_movies': 0, 'action_preference': 0.0}
    
    def _get_user_action_history(self, user_id):
        """
        Get user's action movie history
        """
        query = """
        MATCH (u:User {id: $user_id})-[r:RATED]->(m:Movie)
    ...
    def _get_diverse_popular_movies(self, limit=10):
        ...
        UNWIND ['Action', 'Comedy', 'Drama', 'Thriller', 'Horror', 'Romance', 'Sci-Fi', 'Adventure'] as genre
        MATCH (m:Movie)
    ...
    def _format_movie_results(self, neo4j_result):
        ...
        for record in neo4j_result:
            movie = {
                ...
                'recommendation_score': record.get('final_score', 0.0)
            }
            movies.append(movie)
        return movies
    ...
    def record_user_interaction(self, user_id, movie_id, interaction_type, rating=None, comment=None):
        """
        Record user interaction with intelligent context tracking
        """
        if interaction_type == 'view':
            self._record_view(user_id, movie_id)
        elif interaction_type == 'rating' and rating:
            self._record_rating(user_id, movie_id, rating, comment)
        elif interaction_type == 'watchlist':
            self._record_watchlist(user_id, movie_id)
        elif interaction_type == 'like':
            self._record_like(user_id, movie_id)
    ...
    def _record_view(self, user_id, movie_id):
        ...
        MERGE (u)-[r:VIEWED]->(m)
        SET r.timestamp = datetime(), r.count = coalesce(r.count, 0) + 1
        RETURN r
        """
        self.neo4j.run_query(query, {"user_id": user_id, "movie_id": movie_id})
